...
namespace Mamboo.Internal.Purchase
{
    public class PurchaseController : MonoBehaviour, IStoreListener
    {
        ...
        {
            Debug.Log("ProcessPurchase called");

            // TODO PURCHASE: compare you e.purchasedProduct.definition.id to call needed logic in the game
            if (e.purchasedProduct.definition.id == "YOUR_PRODUCT_ID")
            {
                RemoveAdsInternal();
            }
            else
            {
                Debug.Log("ProcessPurchase NOT success");
            }
        ...
        private void NotifyAdjust(PurchaseEventArgs e)
        {
            ...
            var skuDetails = (Dictionary<string, object>)MiniJson.JsonDecode(gpDetails["skuDetails"].ToString());
            Debug.Log($"skuDetails: {skuDetails}");
            
            var purchaseTime = gpJson.TryGetValue("purchaseTime", out var timeValue) ? ((long)timeValue).ToString(): "";
            var pricemicros = skuDetails.TryGetValue("price_amount_micros", out var micros) ? ((long)micros).ToString() :  "";
        ...
        {
            AdjustPurchase.VerifyPurchaseAndroid(metadata.ProductId, metadata.PurchaseToken, metadata.DeveloperPayload, VerificationInfoDelegate);
        ...
        {
            AdjustPurchase.VerifyPurchaseiOS(metadata.Receipt, metadata.TransactionId, metadata.ProductId, VerificationInfoDelegate);
        ...
        {
            ReportPurchaseTime();
            AppMetrica.Instance.ReportRevenue(new YandexAppMetricaRevenue((decimal)metadata.UnitPrice, metadata.CurrencyCode)
            {
                Quantity = 1,
                ...
                Receipt = new YandexAppMetricaReceipt
                {
                    ...
                    TransactionID = metadata.Platform == PurchasePlatform.ANDROID ? metadata.OrderId : metadata.TransactionId
                }
            });
        }
        ...
        {
            var productWithDollarPrice = ProductsConstants.products.FirstOrDefault(x => x.productId == metadata.ProductId);
            if (productWithDollarPrice != null)
            {
                ...
                FirebaseAnalytics.LogEvent("in_app_purchase_1", new Parameter("value", productWithDollarPrice.priceAndroid), 
                    new Parameter("currency", "USD"));
                FirebaseAnalytics.LogEvent("in_app_purchase_2", new Parameter("value", productWithDollarPrice.priceAndroid), 
                    new Parameter("currency", "USD"));
                FirebaseAnalytics.LogEvent("in_app_purchase_3", new Parameter("value", productWithDollarPrice.priceAndroid), 
                    new Parameter("currency", "USD"));
